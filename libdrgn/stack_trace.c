...
{
	return &frame->trace->states[frame->i];
}
				 ...
				 uint64_t *address_ret, bool *is_reference_ret)
{
	...
	dframe = frame.trace->frames[frame.i];
	state = stack_frame_state(&frame);
...
drgn_stack_frame_parse_parameters(struct drgn_stack_frame frame)
{
	...
	if (state->valid.parameters)
		return NULL;

	err = drgn_program_block_find(prog, pc, &func_die, NULL);
	if (err)
		return err;
...
{
	struct drgn_frame_state *state = stack_frame_state(&frame);
	struct drgn_frame_symbol *symbols = state->parameters.data;
	size_t num_params = state->parameters.size;
	struct drgn_error *err;
...
drgn_stack_frame_parse_variables(struct drgn_stack_frame frame)
{
	...
	struct drgn_error *err;
	uint64_t pc = drgn_stack_frame_pc(frame);
	Dwarf_Die func_die, *cudie, *scopes = NULL;
	Dwfl_Module *module;
	int nscopes, i;
	uint64_t bias;

	if (state->valid.variables)
		return NULL;
...
{
	struct drgn_frame_state *state = stack_frame_state(&frame);
	struct drgn_frame_symbol *symbols = state->variables.data;
	struct drgn_error *err;
	size_t num_symbols = state->variables.size;
