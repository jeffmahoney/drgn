					 ...
					 const Dwfl_Callbacks *callbacks)
{
	...
	omp_init_lock(&dindex->block_tree_lock);
	drgn_dwarf_block_tree_init(&dindex->block_tree);
	c_string_set_init(&dindex->names);
	return NULL;
}

static void drgn_dwarf_block_tree_release(struct drgn_dwarf_block_tree *tree)
{
        struct drgn_dwarf_block_tree_iterator it;

        it = drgn_dwarf_block_tree_first_post_order(tree);
        while (it.entry) {
                struct drgn_dwarf_block_die *entry = it.entry;

                it = drgn_dwarf_block_tree_next_post_order(it);
                free(entry);
        }
}
...
void drgn_dwarf_index_deinit(struct drgn_dwarf_index *dindex)
{
	...
	drgn_dwarf_block_tree_release(&dindex->block_tree);
				      ...
				      uint64_t low_pc, uint64_t high_pc)
{
	...
	omp_set_lock(&dindex->block_tree_lock);
	if (!drgn_dwarf_block_tree_insert(&dindex->block_tree,
					  block_entry, NULL)) {
		omp_unset_lock(&dindex->block_tree_lock);
		free(block_entry);
		return drgn_error_create(DRGN_ERROR_OTHER,
