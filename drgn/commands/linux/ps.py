...
from drgn.helpers.linux.sched import task_state_to_char, task_last_cpu
from drgn.helpers.linux.sched import task_name, task_rss, task_is_kthread
from drgn.helpers.linux.sched import task_is_thread_group_leader
...
import drgn
from drgn.commands import Command, ArgumentParser
...
class TaskFormat:
    ...
    def __init__(self, args: argparse.Namespace,
        ...
        self._filter: Callable[[drgn.Object], bool] = lambda x: True
    ...
    def _format_stack_address(self, task: drgn.Object) -> str:
    ...
    def _format_task_address(self, task: drgn.Object) -> str:
        addr = int(task)
        return f"{addr:16x}"
    ...
    def _format_thread_num(self, task: drgn.Object) -> str:
        return f"{task.thread.num:7d}"

    def _is_kernel_thread(self, task: drgn.Object) -> bool:
        return task_is_kthread(task)

    def _is_user_task(self, task: drgn.Object) -> bool:
        return not task_is_kthread(task)

    def _is_thread_group_leader(self, task: drgn.Object) -> bool:
        return task_is_thread_group_leader(task)

    def _format_common_line(self, task: drgn.Object, state: str) -> str:
        pid = int(task.pid)
        parent_pid = int(task.parent.pid)
        last_cpu = task_last_cpu(task)
        name = task_name(task)
        ...
        if task.mm:
            ...
            total_vm = int(task.mm.total_vm) * 4096 // 1024
            rss = task_rss(task) * 4096 // 1024
    ...
    def _format_last_run(self, task: drgn.Object, state: str) -> str:
        pid = task.task_pid()
    ...
    def should_print_task(self, task: drgn.Object) -> bool:
        """
        ...
        Args:
            task (drgn.Object): The task under consideration
    ...
    def format_one_task(self, task: drgn.Object, state: str) -> str:
        """
        Given the formatting rules, produce the output line for this task.

        Args:
            task (drgn.Object): The task to be printed
...
class PSCommand(Command):
    """display process status information"""
    def __init__(self, name="ps") -> None:
        ...
        super(PSCommand, self).__init__(name, parser)

#        self.task_states: Dict[int, str] = dict()
#
#    def task_state_string(self, task: drgn.Object) -> str:
...
#                task.update_mem_usage()
                musage = self.get_mem_usage(task)
